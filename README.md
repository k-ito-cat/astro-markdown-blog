...
記事の性格は frontmatter の `recordType` で表す。UIでは「記事の型」と呼ぶ。**必須**で、次の5つから1つを選ぶ。公開状態・執筆状態とは別の概念で、読み手が「この記録をどう読めばよいか」を判断するために置く。

- 定義: [src/constants/recordType.ts](src/constants/recordType.ts)

| 値 | 表示 | 意味 |
| --- | --- | --- |
| `research` | 調査 | 一次情報や仕様を調べて理解をまとめた記録 |
| `practice` | 実践 | 自分の環境で実際に作った、導入した記録 |
| `verification` | 検証 | 自分で動かして確かめた記録 |
| `thought` | 思考 | 正解のない問いへの考察と個人的な主張 |
| `impression` | 感想 | 読んだ本、使ったツールの紹介 |

書き方の目安を型ごとに変える。

- `thought` は断定でなくてよい。語り口や曖昧な表現を許す。
- `verification` と `research` は言い切る。根拠として参考文献や脚注を添える。
- `impression` は主観でよい。一般化しない。

型は記事ページ（記録日と同じ行の右端）と、Home・`/blog` の時間軸（行の右端）に、語の下半分を塗るmarkerで表示される。出るのは語だけで、定義はこの表が正本。`/blog` では分野・タグと並ぶ絞り込みの軸になり、`/blog?type=調査` で直接開ける。プレビューの記事一覧では列として並び、行内で変更できる。
