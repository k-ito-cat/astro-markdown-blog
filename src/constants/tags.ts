// MEMO: このファイルがタグの正本。frontmatter のバリデーション（src/content.config.ts）に適用される。
//
// タグは「固有名詞と個別の論点」。表記ゆれを実行時に検知するため enum 運用とし、
// 新しいタグを使うときは必ずこの定数に追加する（追加しないと frontmatter のバリデーションで落ちる）。
export const TAGS = [
  // 言語・記法
  "HTML",
  "CSS",
  "JavaScript",
  "TypeScript",
  "Markdown",
  "Mermaid",
  "SQL",
  "JSON",
  "YAML",
  "TOML",
  // フレームワーク・ライブラリ
  "Astro",
  "React",
  "React Native",
  "Expo",
  "Angular",
  "Vue",
  "Svelte",
  "Next.js",
  "Hono",
  "Kysely",
  "XState",
  "Storybook",
  "zod",
  "TanStack Query",
  "htmx",
  "Tailwind CSS",
  // CMS・ホスティング
  "Headless CMS",
  "microCMS",
  "Decap CMS",
  "Sveltia CMS",
  "Netlify",
  "Vercel",
  "Cloudflare",
  // インフラ・データストア
  "AWS",
  "S3",
  "PostgreSQL",
  "Atlas",
  "データベース",
  // 開発環境
  "Docker",
  "devcontainer",
  "Nix",
  "Devbox",
  "mise",
  "chezmoi",
  "dotfiles",
  "Neovim",
  "WezTerm",
  "Bash",
  "zsh",
  "WSL",
  "Windows",
  "macOS",
  "iOS",
  "Linux",
  "Ubuntu",
  "SSH",
  "1Password",
  "環境変数",
  "IME",
  "Node.js",
  "Bun",
  "リンター",
  "ESLint",
  "Prettier",
  "Biome",
  "Obsidian",
  // バージョン管理・CI
  "Git",
  "GitHub",
  "GitHub Actions",
  "CI/CD",
  "コミット",
  "バージョニング",
  "npm",
  "pnpm",
  // AI
  "MCP",
  "Notion",
  "Cursor",
  "AI活用",
  "Raycast",
  "Claude",
  "Claude Code",
  "Codex",
  "ChatGPT",
  "Copilot",
  "LLM",
  "プロンプト",
  // Web基盤
  "ブラウザ",
  "Chrome",
  "PWA",
  "Picture-in-Picture",
  "HTTP",
  "HTTP/2",
  "HTTP/3",
  "WebSocket",
  "TLS",
  "DNS",
  "CDN",
  "REST API",
  "GraphQL",
  "キャッシュ",
  "SSR",
  "ルーティング",
  "SEO",
  "パフォーマンス",
  "Core Web Vitals",
  "バンドル",
  "ビルド",
  "Vite",
  // 標準・情報源
  "W3C",
  "WHATWG",
  "ECMAScript",
  "RFC",
  "MDN",
  // 表現・アセット
  "画像",
  "WebP",
  "AVIF",
  "JPEG XL",
  "フォント",
  "SVG",
  "ディスプレイ",
  "ハードウェア",
  // UI・UX
  "UIデザイン",
  "UIスタック",
  "デザインシステム",
  "エディトリアルデザイン",
  "グリッドシステム",
  "アクセシビリティ",
  "レスポンシブ",
  "フォーム",
  "ページネーション",
  "Figma",
  // セキュリティ
  "認証・認可",
  "OAuth",
  "JWT",
  "Cookie",
  "CORS",
  "プライバシー",
  "法規制",
  "消費者保護",
  "GDPR",
  "電気通信事業法",
  "サプライチェーン",
  "脆弱性",
  // 設計・品質
  "モノレポ",
  "マルチテナント",
  "ディレクトリ設計",
  "型安全",
  "状態管理",
  "非同期処理",
  "エラーハンドリング",
  "コーディング規約",
  "テスト",
  "Vitest",
  "Playwright",
  "負荷テスト",
  "監視",
  "GA4",
  "Sentry",
  "アルゴリズム",
  "数学",
  // プロセス
  "レビュー",
  "見積もり",
  "ファシリテーション",
  "タスク管理",
  "ドキュメント",
  "生産性",
  "プロダクト設計",
  "用語",
  "検証",
  "MDX",
  "コンポーネント設計",
  "詳細度",
  "WCAG",
  "WAI-ARIA",
  "タイポグラフィ",
  "モジュラースケール",
  "ビット演算",
  "React Router",
  "仮想DOM",
  "レンダリング",
  "文字コード",
  "Base64",
  "OGP",
  "スタックトレース",
] as const;
export type Tag = (typeof TAGS)[number];

/*
 * 五十音順へ並べるための読み。漢字で始まるタグだけ持つ
 * （かな・ラテン文字のタグは表記そのものから並び順を決められる）。
 * 漢字で始まるタグを TAGS へ足すときは、ここにも読みを足す
 */
export const TAG_YOMI = {
  環境変数: "かんきょうへんすう",
  画像: "がぞう",
  "認証・認可": "にんしょうにんか",
  法規制: "ほうきせい",
  消費者保護: "しょうひしゃほご",
  電気通信事業法: "でんきつうしんじぎょうほう",
  脆弱性: "ぜいじゃくせい",
  型安全: "かたあんぜん",
  状態管理: "じょうたいかんり",
  非同期処理: "ひどうきしょり",
  負荷テスト: "ふかてすと",
  監視: "かんし",
  数学: "すうがく",
  見積もり: "みつもり",
  生産性: "せいさんせい",
  用語: "ようご",
  検証: "けんしょう",
  詳細度: "しょうさいど",
  仮想DOM: "かそうどむ",
  文字コード: "もじこーど",
} as const satisfies Partial<Record<Tag, string>>;
